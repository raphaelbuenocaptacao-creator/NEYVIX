...
const checks = [
  ...
  ["drive detects schema readiness on operational failure", /inspectDriveDocsRepair/.test(files.api) && /readiness\.drive !== "ready"/.test(files.api)],
  ["drive exposes stable schema-not-ready code", /SCHEMA_NOT_READY/.test(files.api) && /module: "drive"/.test(files.api)],
  ["drive degraded response remains private and retryable", /Retry-After/.test(files.api) && /status: 503/.test(files.api)],
  ["drive distinguishes generic service outage", /SERVICE_UNAVAILABLE/.test(files.api)],
