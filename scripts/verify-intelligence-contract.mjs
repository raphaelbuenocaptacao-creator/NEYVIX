...
const checks = [
  ...
  ["memory context request is explicit opt-in and prompt-scoped", route.includes('(body as { useMemory?: unknown }).useMemory === true') && route.includes("loadAiMemoryContext(session.email, useMemory, prompt, 8)")],
  ...
  ["memory recall uses prompt-aware selector", aiMemoryContext.includes("getRelevantMemoryContext(email, query, limit)")],
