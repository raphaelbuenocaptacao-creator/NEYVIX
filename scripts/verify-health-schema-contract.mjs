...
const checks = [
  ...
  ["health exposes Docs schema status", /docs: "ready" \| "missing" \| "unknown"/.test(health)],
  ...
  ["health computes Docs readiness", /const docsReady = Boolean\(catalog\.documents_table\)/.test(health)],
  ["health reports productRecords and Docs in schema payload", /schema:\s*\{[\s\S]*automation[\s\S]*memory[\s\S]*productRecords[\s\S]*docs[\s\S]*ecosystem/.test(health)],
  ["launch readiness requires complete product records and Docs", /launchReady:[\s\S]*productRecords === "ready"[\s\S]*docsReady[\s\S]*ecosystem === "ready"/.test(health)],
];
