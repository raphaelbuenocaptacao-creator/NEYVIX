...
export default async function AutomationPage() {
  ...
  return <main className="shell">
    ...
    <section className="grid" aria-label="Automações recentes">{workspace.automations.length ? workspace.automations.map((automation) => <article key={automation.id}><span>{statusLabel[automation.status] ?? automation.status}</span><h2>{automation.name}</h2><p>{automation.triggerType} → {automation.actionType}</p><p>Atualizada em {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(automation.updatedAt))}</p></article>) : <article><span>01</span><h2>Nenhuma automação ativa</h2><p>Crie o primeiro fluxo assim que a base estiver disponível.</p></article>}</section>
