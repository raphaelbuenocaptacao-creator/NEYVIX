...
export async function POST(request: Request) {
  ...
  const id = await upsertMemory({ email: session.email, key, category, value, isPrivate: true, source: "user" });
  if (!id) return NextResponse.redirect(new URL("/memory?error=unavailable", request.url), 303);
  return NextResponse.redirect(new URL("/memory?saved=1", request.url), 303);
}
